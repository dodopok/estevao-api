      ...
      class Midday < Base
        ...
        def assemble_office
          if preferences[:office_type] == "family"
            assemble_midday_prayer_family
          else
            assemble_midday_prayer
          end
        end

        def assemble_midday_prayer_family
          [
            build_family_midday_opening,
            build_family_psalms,
            build_family_midday_reading,
            build_lords_prayer_simple,
            build_family_midday_collect
          ].flatten.compact
        end

        def build_family_midday_opening
          text = fetch_liturgical_text("family_midday_opening")
        ...
        def build_family_psalms
          psalm = fetch_liturgical_text("family_midday_psalm_121")
          return nil unless psalm

          {
            name: "Levavi Oculus (Salmo 121)",
            slug: "psalms",
            lines: [
        ...
        def build_family_midday_reading
          reading_num = resolve_preference(:family_midday_reading, 1..3) || 1
          text = fetch_liturgical_text("family_midday_reading_#{reading_num}")
          return nil unless text
        ...
        def build_family_midday_collect
          text = fetch_liturgical_text("family_midday_collect")
          return nil unless text
        ...
        def build_opening
          ...
          if preparation
            lines << line_item(preparation.content, type: "leader", slug: preparation.slug)
            lines << line_item("", type: "spacer")
          end
        ...
        def build_invitation
          ...
          if invitation
            lines << line_item(invitation.content, type: "responsive", slug: invitation.slug)
            lines << line_item("", type: "spacer")
          end
        ...
        def build_psalms
          ...
          Array(selected_psalms).each do |psalm_num|
            ...
            if psalm
              lines = []
              lines << line_item(psalm.content, type: "congregation", slug: psalm.slug)
        ...
        def build_readings
          ...
          (1..3).each do |reading_num|
            ...
            if reading
              lines << line_item(reading.content, type: "leader", slug: reading.slug)
              lines << line_item("", type: "spacer")
            end
          end
        ...
        def build_prayer
          ...
          if our_father
            lines << line_item(our_father.content, type: "responsive", slug: our_father.slug)
            lines << line_item("", type: "spacer")
          end
          ...
          if rubric_after
            lines << line_item(rubric_after.content, type: "rubric", slug: rubric_after.slug)
            lines << line_item("", type: "spacer")
          end
          ...
          if prayer
            lines << line_item(prayer.content, type: "responsive", slug: prayer.slug)
            lines << line_item("", type: "spacer")
          end
        ...
        def build_dismissal
          ...
          if dismissal
            lines << line_item(dismissal.content, type: "responsive", slug: dismissal.slug)
          end
