...
class DailyOfficeService
  ...
  def assemble_morning_prayer
    return assemble_morning_prayer_family if @prefs[:family_rite]
  ...
  def assemble_morning_prayer_family
    # Rito familiar simplificado para Oração Matutina (8 componentes)
    [
      build_opening_sentence,
      build_invocation,
      build_single_psalm,
      ...
      build_kyrie,
      build_lords_prayer,
      build_simple_collect,
      build_dismissal
    ].compact
  end

  def assemble_evening_prayer
    return assemble_evening_prayer_family if @prefs[:family_rite]
  ...
  def assemble_evening_prayer_family
    # Rito familiar simplificado para Oração Vespertina (8 componentes)
    [
      build_opening_sentence,
      build_invocation,
      build_single_psalm_evening,
      build_first_reading,
      build_kyrie,
      build_lords_prayer,
      build_simple_collect,
  ...
  # Métodos simplificados para rito familiar
  def build_single_psalm
    cycle = PsalmCycle.find_for_date_and_office(@date, "morning")
    return nil unless cycle

    # Pega apenas o primeiro salmo do ciclo
    psalm_numbers = cycle.psalm_numbers
    first_psalm_number = psalm_numbers.first
  ...
  def build_single_psalm_evening
    cycle = PsalmCycle.find_for_date_and_office(@date, "evening")
    return nil unless cycle
  ...
  def build_single_psalm_lines(psalm_number)
    psalm = Psalm.find_psalm(psalm_number, prayer_book_code: @prefs[:prayer_book_code])
    return [] unless psalm
  ...
  def build_simple_collect
    # Apenas a coleta do dia para rito familiar
