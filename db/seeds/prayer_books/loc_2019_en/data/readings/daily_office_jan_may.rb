# ================================================================================
# DAILY OFFICE LECTIONARY (JAN-MAY) - LOC 2019 EN (English)
# ================================================================================

Rails.logger.info "📖 Loading Daily Office readings (Jan-May) for LOC 2019 EN..."
...
def seed_month(month_name, days_count, mp_map, ep_map, prayer_book_id)
  # Map Portuguese month names to English for consistency with existing seeds if needed,
  # but here we just need the month index. Actually, existing seeds use "june", "july".
  # I'll convert the month_name to english for the file reference generation if it's not already.
  ...
  eng_month_name = month_map[month_name] || month_name

  (1..days_count).each do |day|
    date_ref = "#{eng_month_name}_#{day}"
...
seed_month("january", 31, jan_mp, jan_ep, prayer_book.id)
seed_month("february", 29, feb_mp, feb_ep, prayer_book.id)
seed_month("march", 31, mar_mp, mar_ep, prayer_book.id)
seed_month("april", 30, apr_mp, apr_ep, prayer_book.id)
seed_month("may", 31, may_mp, may_ep, prayer_book.id)
...
Rails.logger.info "✅ Daily Office readings (Jan-May) loaded for LOC 2019 EN!"
