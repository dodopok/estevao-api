# ================================================================================
# SEED DE CELEBRAÇÕES (Principal Feasts, Holy Days, Commemorations) - LOC 1987
# Carrega dados dos arquivos JSON e insere/atualiza no banco
# ================================================================================

Rails.logger.info "📅 Carregando celebrações do calendário LOC 1987..."

CALENDAR_PATH = File.dirname(__FILE__)
PRAYER_BOOK_CODE = 'loc_1987'
...
def bulk_upsert_celebrations(celebrations_data, prayer_book, type_name)
  ...
  default_rank = DEFAULT_RANKS[celebrations_data.first['celebration_type']] || 300
...
begin
  ActiveRecord::Base.transaction do
    ...
    bulk_upsert_celebrations(major_holy_days, prayer_book, 'Dias Santos')
    ...
    # Carrega e processa Commemorations
    commemorations = load_json_file('commemorations.json')
    bulk_upsert_celebrations(commemorations, prayer_book, 'Comemorações')
  end

  total = Celebration.where(prayer_book_id: prayer_book.id).count
  Rails.logger.info "✅ Calendário LOC 1987 carregado! Total: #{total} celebrações"
