...
RSpec.describe 'Daily Office Premium Authorization', type: :request do
  let!(:free_book) { create(:prayer_book, code: 'test_office_free', name: 'Free Office', premium_required: false, language: 'pt-BR') }
  let!(:premium_book) { create(:prayer_book, code: 'test_office_premium', name: 'Premium Office', premium_required: true, language: 'pt-BR') }
  ...
  describe 'Premium authorization behavior' do
    before do
      ...
      allow_any_instance_of(Api::V1::Concerns::PreferencesResolver).to receive(:resolved_preferences).and_return({
                                                                                                                      prayer_book_code: 'test_office_premium',
                                                                                                                      bible_version: 'nvi',
                                                                                                                      language: 'pt-BR'
                                                                                                                    })
    end
    ...
    context 'when user is not premium' do
      ...
      it 'includes prayer book information in error' do
        ...
        expect(json[:error][:prayer_book_code]).to eq('test_office_premium')
        expect(json[:error][:premium_required]).to be true
        expect(json[:error][:message]).to include('Premium Office')
      end
    end
    ...
    context 'when user is premium' do
      ...
      before do
        ...
        mock_response = {
          ...
          office_type: 'morning',
          metadata: { prayer_book_code: 'test_office_premium' }
        }
  ...
  describe 'Authorization with free prayer book' do
    before do
      ...
      allow_any_instance_of(Api::V1::Concerns::PreferencesResolver).to receive(:resolved_preferences).and_return({
                                                                                                                      prayer_book_code: 'test_office_free',
                                                                                                                      bible_version: 'nvi',
                                                                                                                      language: 'pt-BR'
                                                                                                                    })
